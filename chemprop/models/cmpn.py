...
class CMPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        if self.args.add_step == 'func_frag_attention':
            ...
            #self.funcional_group_embedding = nn.Embedding(90,64)
    ...
    def forward(self, step, mol_graph, features_batch=None) -> torch.FloatTensor:
        ...
        if self.args.step == 'BRICS_prompt':# 使用高层次BRICS片段进行反应
...
class FunctionalGroupEmbedding(nn.Module):
    def __init__(self, num_groups, features_dim):
        ...
        # import pickle
        '''fg2emb = pickle.load(open('./embedding/fg2emb.pkl', 'rb'))
        fg2emb = [torch.tensor(i.tolist()).unsqueeze(0) for i in fg2emb.values()]
        fg2emb = torch.cat(fg2emb,dim=0)
        self.embedding = fg2emb'''
        # df = pd.read_csv("./chemprop/data/funcgroup.csv")
        # smiles_lst = df['smiles'].to_list()  # 示例的分子结构
        # name_lst = df['name'].to_list()
        # smart_lst = df['smart'].to_list()
        # generator = RDKit2DNormalized()
        # features_map = Pool(1).imap(generator.process, smiles_lst)
        #funcgembedding = np.load("./chemprop/data/func_descriptor.npy")[:,1:]
        # funcgembedding = []
        # for i,(name,smart,smiles) in enumerate(zip(name_lst,smart_lst,smiles_lst)):
        #     mol = Chem.MolFromSmiles(smiles)
        #     FP = list(Chem.RDKFingerprint(mol, minPath=1, maxPath=7, fpSize=64))
        #     funcgembedding.append(FP)
        self.embedding = nn.Parameter(torch.Tensor(num_groups,features_dim),requires_grad=True)
        # self.embedding = nn.Parameter(torch.FloatTensor(num_groups,features_dim),requires_grad=True)
    ...
    def reset_parameters(self):
        ...
        # for i,(name,smart,smiles) in enumerate(zip(name_lst,smart_lst,smiles_lst)):
        #     mol = Chem.MolFromSmiles(smiles)
        #     FP = list(Chem.RDKFingerprint(mol, minPath=1, maxPath=7, fpSize=get_atom_fdim()))
        #     self.embedding[i] = nn.Parameter(torch.FloatTensor(FP),requires_grad=True)
    def forward(self, group_indices):
        # 通过索引获取对应的特征向量
        #return F.embedding(group_indices, self.embedding)
        return self.embedding[group_indices].cuda()
