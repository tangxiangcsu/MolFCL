...
from .cmpn_dgl import CMPNDGL
from .cmpn import CMPN
from .mpn import MPN
from .HyperGraph_BRICS import HyperGraph
from .cmpnn_only_atom import CMPN_ONLY_ATOM
import torchvision
...
class MoleculeModel(nn.Module):
    ...
    def create_encoder(self, args: Namespace, encoder_name):
        ...
        elif encoder_name == 'HyperGraph':
            self.encoder = HyperGraph(args)
        elif encoder_name == 'CMPNDGL':
            self.encoder = CMPNDGL(args)
        elif encoder_name == 'CMPNN_ONLY_ATOM':
            self.encoder = CMPN_ONLY_ATOM(args)
    def create_ffn(self, args: Namespace):
        ...
        dropout = nn.Dropout(args.ffn_drop_out)
...
def build_t_SNEModel(args:Namespace,encoder_name)->nn.Module:
    """
...
class AttentionLayer(nn.Module):
    def __init__(self, args):
        ...
        self.dropout = nn.Dropout(args.prompt_drop_out)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V,max_fragment_size, attn_mask):
        '''
        input_Q: [batch_size, len_q, d_model]
        input_K: [batch_size, len_k, d_model]
        input_V: [batch_size, len_v(=len_k), d_model]
        attn_mask: [batch_size, max_fragment, seq_len, seq_len]
        '''
        attn_mask = attn_mask.unsqueeze(2).repeat(1,1,self.num_heads,1,1)
        residual, batch_size = input_Q, input_Q.size(0)
        # (B, S, D) -proj-> (B, S, D_new) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        Q = self.W_Q(input_Q).view(batch_size, max_fragment_size, -1, self.num_heads, self.depth).transpose(2,3)  # Q: [batch_size, max_frag, n_heads, len_q, d_k]
        K = self.W_K(input_K).view(batch_size, max_fragment_size, -1, self.num_heads, self.depth).transpose(2,3)  # K: [batch_size, max_frag, n_heads, len_k, d_k]
        V = self.W_V(input_V).view(batch_size, max_fragment_size, -1, self.num_heads, self.depth).transpose(2,3)  # V: [batch_size, max_frag, n_heads, len_v(=len_k), d_v]

        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
        context, attn = self.dot_product_attention(Q, K, V, attn_mask)
        context = context.transpose(2,3).reshape(batch_size, max_fragment_size, -1, self.d_model) # context: [batch_size, max_frag, len_q, n_heads * d_v]
...
class Prompt_generator(nn.Module):
    def __init__(self, args:Namespace):
        ...
        self.dropout = args.prompt_drop_out
        ...
        # Dropout
        # self.W_fg = nn.Linear(512,self.hidden_size*3)
        ...
        #self.linear_fg = nn.Linear(self.hidden_size,self.hidden_size * 3)
    ...
    def forward(self, f_atom, atom_scope, f_group, group_scope, mapping, mapping_scope):
        # descriptors = self.act_func(self.W_fg(descriptors))
        ...
        for i,(g_start, g_size) in enumerate(group_scope):
            ...
            cur_a = cur_a[cur_m]
            # 找到非零行的索引
            '''non_zero_rows = (cur_a != 0)
            sum_along_dim1 = (cur_a * non_zero_rows.float()).sum(dim=1)
            count_along_dim1 = non_zero_rows.sum(dim=1, dtype=torch.float32)
            cur_fg_hidden = sum_along_dim1 / count_along_dim1
            cur_fg_hidden[count_along_dim1==0]=0'''
            # cur_fg_hidden = cur_a.sum(dim=0)
            # cur_fg_message = cur_a.sum(dim=1) * cur_a.max(dim=1)[0]
            cur_g = torch.cat([cur_a.sum(dim=1),cur_a.max(dim=1)[0],cur_g],dim=1)
            #cur_g = torch.cat([cur_fg_message,cur_g],dim=1)
            cur_brics = torch.cat([self.fg,cur_g],dim=0)
            # cur_brics = torch.cat([descriptors[i].unsqueeze(0),cur_g],dim=0)
        ...
        # padding_mask = (f_frag_list != 0) + 0.0
        # mask = torch.matmul(padding_mask, padding_mask.transpose(-2, -1))
...
def add_BRICS_prompt(model:nn.Module,args:Namespace = None):
