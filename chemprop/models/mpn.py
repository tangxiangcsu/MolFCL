    ...
    def forward(self, mol_graph, feature_batch: List[np.ndarray] = None) -> torch.FloatTensor:
        ...
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, atom_num,\
            f_brics, f_reacts, p2r, r2p, r2revb, p_scope, pharm_num,\
                f_grop, grop_scope, grop_num= mol_graph.get_components()
        assert len(a_scope)==len(p_scope)
        if self.args.cuda or next(self.parameters()).is_cuda:
            f_atoms, f_bonds, a2b, b2a, b2revb,f_brics, f_reacts, p2r, r2p, r2revb, f_grop = (
...
class MPN(nn.Module):
    ...
    def forward(self,step, pretrain: bool,
                batch,
