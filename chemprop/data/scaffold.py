...
def scaffold_split(data: MoleculeDataset,
    ...
    if balanced:  # Put stuff that's bigger than half the val/test size into train, rest just order randomly
        ...
        random.seed(seed)
        random.shuffle(big_index_sets)# 大于验证集或者测试集的一半的分子支架
        random.shuffle(small_index_sets)# 小于
    ...
    train_idx,valid_idx,test_idx = train,val,test
    # Map from indices to data
    train = [data[i] for i in train]
    val = [data[i] for i in val]
    test = [data[i] for i in test]
    # np.save('./data/toxcast/toxcast-scaffold-2.npy',[train_idx,valid_idx,test_idx])
    return MoleculeDataset(train), MoleculeDataset(val), MoleculeDataset(test)
